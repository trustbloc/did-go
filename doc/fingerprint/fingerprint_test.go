...
func TestCreateDIDKey(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			keyCode:  fingerprint.BLS12381g1g2PubKeyMultiCodec,
		},
	}

	for _, test := range tests {
		tc := test
		t.Run(tc.name+" CreateED25519DIDKey", func(t *testing.T) {
			keyBytes := base58.Decode(tc.keyB58)
			// append G1G2 public keys for Creation of DIDKey for BLS12381g1g2PubKeyMultiCodec
			if tc.keyCode == fingerprint.BLS12381g1g2PubKeyMultiCodec {
				g1Bytes := base58.Decode(bbsPubKeyG1Base58)
				keyBytes = append(g1Bytes, keyBytes...)
			}

			didKey, keyID := fingerprint.CreateDIDKeyByCode(tc.keyCode, keyBytes)
		...
		t.Run(tc.name+" PubKeyFromFingerprint success", func(t *testing.T) {
			pubKey, code, err := fingerprint.PubKeyFromFingerprint(strings.Split(tc.DIDKeyID, "#")[1])
		...
		t.Run(tc.name+" PubKeyFromDIDKey", func(t *testing.T) {
			pubKey, err := fingerprint.PubKeyFromDIDKey(tc.DIDKey)
			if tc.crv != nil {
	...
	t.Run("test PubKeyFromFingerprint fail", func(t *testing.T) {
		...
		_, _, err := fingerprint.PubKeyFromFingerprint(badDIDKeyID)
		require.EqualError(t, err, "unknown key encoding")
	})

	t.Run("invalid fingerprint", func(t *testing.T) {
		_, _, err := fingerprint.PubKeyFromFingerprint("")
		require.Error(t, err)

		_, _, err = fingerprint.PubKeyFromFingerprint("a6MkpTHR8VNsBxYAAWHut2Geadd9jSwuBV8xRoAnwWsdvktH")
		require.Error(t, err)
	})
}
...
func TestCreateDIDKeyByJwk(t *testing.T) {
	...
	for _, test := range tests {
		...
		t.Run(tc.name+" CreateDIDKeyByJwk", func(t *testing.T) {
			...
			case "test Ed25519":
				...
				jwkKey = jwksupport.FromEdPublicKey(edKey)
				require.NoError(t, err)
			case "test X25519":
			...
			default:
				x := readBigInt(t, tc.x)
				y := readBigInt(t, tc.y)
				publicKey := jwksupport.EcdsaContent{
					Curve: tc.curve,
					X:     x,
					Y:     y,
				}

				jwkKey, err = jwksupport.FromEcdsaContent(publicKey)

				if tc.name == "test EC with invalid curve" {
					require.EqualError(t, err, "unsupported/unknown elliptic curve")
					jwkKey = &jwk.JWK{
						Kty: "EC",
						Crv: "invalid",
					}
			...
			didKey, keyID, err := jwksupport.CreateDIDKeyByJwk(jwkKey)
	...
	t.Run("nil input", func(t *testing.T) {
		_, _, err := jwksupport.CreateDIDKeyByJwk(nil)
		require.Error(t, err)
	...
	t.Run("test invalid type", func(t *testing.T) {
		...
		_, _, err := jwksupport.CreateDIDKeyByJwk(&jwkKey)
		require.Error(t, err)
...
func TestDIDKeyEd25519(t *testing.T) {
	...
	didKey, keyID := fingerprint.CreateED25519DIDKey(base58.Decode(k1Base58))

	require.Equal(t, didKey, k1)
	require.Equal(t, keyID, k1KeyID)

	pubKey, err := fingerprint.PubKeyFromDIDKey(k1)
...
func TestDIDKeyX25519(t *testing.T) {
	...
	didKey, keyID := fingerprint.CreateDIDKeyByCode(fingerprint.X25519PubKeyMultiCodec, base58.Decode(x25519Base58))

	require.Equal(t, x25519DIDKey, didKey)
	require.Equal(t, keyID, keyIDX25519)

	pubKey, err := fingerprint.PubKeyFromDIDKey(x25519DIDKey)
	require.NoError(t, err)
	require.Equal(t, x25519Base58, base58.Encode(pubKey))
}

func TestPubKeyFromDIDKeyFailure(t *testing.T) {
	_, err := fingerprint.PubKeyFromDIDKey("did:key:****")
