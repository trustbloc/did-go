...
func TestVDRI_Create(t *testing.T) {
	...
	t.Run("test endpoint down", func(t *testing.T) {
		...
		v, err := New(&mockKeyRetriever{}, WithHTTPClient(&http.Client{}),
			WithDomain("domain1.com"), WithDomain("domain2.com"))
		require.NoError(t, err)
...
func TestVDRI_Read(t *testing.T) {
	...
	t.Run("test failed for tracing opt", func(t *testing.T) {
		v, err := New(nil)
		require.NoError(t, err)

		v.getHTTPVDR = httpVdrFunc(&did.DocResolution{DIDDocument: &did.Doc{ID: "did"}}, nil)

		_, err = v.Read("did", vdrapi.WithOption(ResolutionEndpointsOpt, []string{"url"}),
			vdrapi.WithOption(TracingCtxOpt, "span"))
		require.Error(t, err)
		require.Contains(t, err.Error(), "tracingOpt not type of span")
	})
