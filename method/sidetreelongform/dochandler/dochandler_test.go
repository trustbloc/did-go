...
func TestResolve(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		require.Equal(t,
			didDoc[document.KeyPurposeCapabilityDelegation],
			expectedIONDoc[document.KeyPurposeCapabilityDelegation])
		require.Equal(t,
			didDoc[document.KeyPurposeCapabilityInvocation],
			expectedIONDoc[document.KeyPurposeCapabilityInvocation])
		require.Equal(t, didDoc[document.KeyPurposeKeyAgreement], expectedIONDoc[document.KeyPurposeKeyAgreement])

		require.Equal(t,
			didDoc.VerificationMethods()[0].Controller(),
		...
		require.Equal(t,
			didDoc.VerificationMethods()[0].PublicKeyJwk(),
			expectedIONDoc.VerificationMethods()[0].PublicKeyJwk())
