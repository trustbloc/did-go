...
type Protocol struct {
	...
	// It has to be greater than max delta size (big) + max proof size (medium) + other small
	// values (operation type, suffix-data)
	...
	// SignatureAlgorithms contain supported signature algorithms for
	// signed operations (e.g. EdDSA, ES256, ES384, ES512, ES256K).
	SignatureAlgorithms []string `json:"signatureAlgorithms"`

	// KeyAlgorithms contain supported key algorithms for signed
	// operations (e.g. secp256k1, P-256, P-384, P-512, Ed25519).
	KeyAlgorithms []string `json:"keyAlgorithms"`

	// MaxOperationTimeDelta is maximum time that operation should be valid before
	// it expires; used with anchor from time
