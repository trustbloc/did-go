...
// Cut returns the current batch along with number of items that should be remaining in the queue after the committer
// is called. If force is false then the batch will be cut only if it has reached the max batch size (as specified
// in the protocol). If force is true then the batch will be cut if there is at least one Data in the batch
// Note that the operations are removed from the queue when Result.Ack is invoked,
// otherwise Result.Nack should be called in order to place the operations back in the queue so that
...
func (r *BatchCutter) Cut(force bool) (Result, error) {
	...
	if err != nil {
		return Result{Pending: pending}, nil //nolint: nilerr
	}
...
// getOperationsAtProtocolVersion iterates through the operations and returns the operations
// which are at the same protocol genesis time.
func getOperationsAtProtocolVersion(opsAtTime []*operation.QueuedOperationAtTime,
) ([]*operation.QueuedOperation, uint64) {
	var (
	...
	for _, op := range opsAtTime {
		...
		if op.ProtocolVersion != protocolVersion {
			...
			logger.Info("Not adding operation since its protocol genesis time is different from the "+
				"protocol genesis time of the existing ops in the batch.",
				logfields.WithOperationGenesisTime(op.ProtocolVersion),
