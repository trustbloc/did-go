...
import (
	...
	"github.com/trustbloc/did-go/method/sidetreelongform/sidetree-core/batch/cutter"
...
type AnchorWriter interface {
	...
	WriteAnchor(
		anchor string, artifacts []*protocol.AnchorDocument, ops []*operation.Reference, protocolVersion uint64) error
...
func (r *Writer) processAvailable(forceCut bool) uint {
	...
	r.logger.Debug("Forcefully processing operations",
		logfields.WithTotalPending(pending))

	// Now process the remaining operations
	n, pending, err := r.cutAndProcess(true)
	if err != nil {
		r.logger.Warn("Error processing operations", log.WithError(err),
			logfields.WithTotalPending(pending))
	} else {
		r.logger.Info("Successfully processed operations.", logfields.WithTotal(n),
			logfields.WithTotalPending(pending))
	}
...
func (r *Writer) drain() (pending uint, err error) {
	for {
		...
		if err != nil {
			r.logger.Error("Error draining operations: cutting and processing returned an error",
				log.WithError(err))
		...
		r.logger.Info(" ... drain processed operations into batch.",
			logfields.WithTotal(n), logfields.WithTotalPending(pending))
	}
}
