...
//
//nolint:funlen
func (r *DocumentHandler) ProcessOperation(
	operationBuffer []byte, protocolVersion uint64) (*document.ResolutionResult, error) {
	startTime := time.Now()
	...
	if err != nil {
		return nil, fmt.Errorf(
			"failed to add operation for suffix[%s] to unpublished operation store: %s",
...
func (r *DocumentHandler) getUnpublishedOperation(
	op *operation.Operation, pv protocol.Version) *operation.AnchoredOperation {
...
func (r *DocumentHandler) getCreateResponse(
	op *operation.Operation, pv protocol.Version) (*document.ResolutionResult, error) {
	startTime := time.Now()
...
func GetTransformationInfoForUnpublished(
	namespace, domain, label, suffix, createRequestJCS string) protocol.TransformationInfo {
