...
func (o *Observer) process(txns []txn.SidetreeTxn) {
	for _, txn := range txns {
		...
		if err != nil {
			logger.Warn(
				"Failed to get protocol client for namespace",
				logfields.WithNamespace(txn.Namespace), log.WithError(err))
		...
		if err != nil {
			logger.Warn(
				"Failed to get processor for transaction time", logfields.WithGenesisTime(txn.ProtocolVersion),
				log.WithError(err))
		...
		if err != nil {
			logger.Warn("Failed to process anchor",
				logfields.WithAnchorString(txn.AnchorString), log.WithError(err))
