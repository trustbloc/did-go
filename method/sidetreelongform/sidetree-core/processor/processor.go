...
import (
	...
	logfields "github.com/trustbloc/did-go/method/sidetreelongform/sidetree-core/internal/log"
)

const loggerModule = "sidetree-core-processor"

// OperationProcessor will process document operations in chronological order and create final
// document during resolution.
...
// nolint:funlen
func (s *OperationProcessor) Resolve(uniqueSuffix string,
	opts ...document.ResolutionOption) (*protocol.ResolutionModel, error) {
	...
	publishedOps, unpublishedOps, filteredOps, err :=
		s.processOperations(publishedOps, unpublishedOps, uniqueSuffix, opts...)
	if err != nil {
		return nil, err
	}
	...
	if len(fullOps) > 0 {
		s.logger.Debug("Applying full operations", logfields.WithTotal(len(fullOps)),
	...
	filteredUpdateOps := getOpsWithTxnGreaterThanOrUnpublished(
		updateOps, rm.LastOperationTransactionTime, rm.LastOperationTransactionNumber)
	if len(filteredUpdateOps) > 0 {
		s.logger.Debug("Applying update operations after last full operation",
			logfields.WithTotal(len(filteredUpdateOps)),
			logfields.WithSuffix(uniqueSuffix))
...
func (s *OperationProcessor) processOperations(
	...
	if err != nil {
		return nil, nil, nil,
			fmt.Errorf("failed to apply resolution options for document id[%s]: %s", uniqueSuffix, err.Error())
	}
...
func filterOpsByVersionID(ops []*operation.AnchoredOperation, versionID string,
...
func filterOpsByVersionTime(ops []*operation.AnchoredOperation, timeStr string,
) ([]*operation.AnchoredOperation, error) {
...
func (s *OperationProcessor) applyResolutionOptions(uniqueSuffix string, published,
	unpublished []*operation.AnchoredOperation, opts document.ResolutionOptions,
) ([]*operation.AnchoredOperation, []*operation.AnchoredOperation, []*operation.AnchoredOperation, error) {
	...
	var (
		filteredPublishedOps   []*operation.AnchoredOperation
...
func (s *OperationProcessor) createOperationHashMap(ops []*operation.AnchoredOperation,
	...
	for _, op := range ops {
		...
		if err != nil {
			s.logger.Info("Skipped bad operation while creating operation hash map",
		...
		if err != nil {
			s.logger.Info("Skipped calculating commitment while creating operation hash map",
				logfields.WithSuffix(op.UniqueSuffix),
				logfields.WithOperationType(string(op.Type)), logfields.WithTransactionTime(op.TransactionTime),
				logfields.WithTransactionNumber(op.TransactionNumber), log.WithError(err))
...
func splitOperations(ops []*operation.AnchoredOperation,
...
func getOpsWithTxnGreaterThanOrUnpublished(ops []*operation.AnchoredOperation, txnTime, txnNumber uint64,
...
func (s *OperationProcessor) applyOperations(ops []*operation.AnchoredOperation, rm *protocol.ResolutionModel,
	...
	for ok {
		...
		s.logger.Debug("Successfully processed commitment",
			logfields.WithCommitment(c), logfields.WithSuffix(uniqueSuffix))

		// get next commitment to be processed
		c = commitmentFnc(state)

		s.logger.Debug("Next commitment to process",
			logfields.WithCommitment(c), logfields.WithSuffix(uniqueSuffix))
	...
	for _, op := range createOps {
		...
		if state, err = s.applyOperation(op, rm); err != nil {
			s.logger.Info(
				...
				logfields.WithSuffix(op.UniqueSuffix),
				logfields.WithOperationType(string(op.Type)),
				logfields.WithTransactionTime(op.TransactionTime),
				logfields.WithTransactionNumber(op.TransactionNumber),
				log.WithError(err))
		...
		s.logger.Debug(
			"Applied create operation, recover commitment, update commitment which results in new document",
...
func (s *OperationProcessor) applyFirstValidOperation(ops []*operation.AnchoredOperation, rm *protocol.ResolutionModel,
	...
	for _, op := range ops {
		...
		if err != nil {
			s.logger.Info("Skipped bad operation",
				logfields.WithSuffix(op.UniqueSuffix),
				logfields.WithOperationType(string(op.Type)),
				logfields.WithTransactionTime(op.TransactionTime),
				logfields.WithTransactionNumber(op.TransactionNumber), log.WithError(err))
		...
		if currCommitment == nextCommitment {
			s.logger.Info(
				"Skipped bad operatio. Reason: operation commitment(key) equals next operation commitment(key)",
				logfields.WithSuffix(op.UniqueSuffix),
				logfields.WithOperationType(string(op.Type)),
				logfields.WithTransactionTime(op.TransactionTime),
				logfields.WithTransactionNumber(op.TransactionNumber))
		...
		s.logger.Debug("Applyied operation.",
			logfields.WithOperation(op),
			logfields.WithRecoveryCommitment(state.RecoveryCommitment),
			logfields.WithUpdateCommitment(state.UpdateCommitment),
			logfields.WithDeactivated(state.Deactivated), logfields.WithDocument(state.Doc))
