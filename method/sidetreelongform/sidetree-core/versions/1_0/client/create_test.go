...
func TestNewCreateRequest(t *testing.T) {
	...
	t.Run("recovery commitment error", func(t *testing.T) {
		request, err := NewCreateRequest(
			&CreateRequestInfo{OpaqueDocument: "{}", RecoveryCommitment: recoveryCommitment})
		require.Error(t, err)
		require.Empty(t, request)
		require.Contains(t, err.Error(),
			"recovery commitment is not computed with the specified hash algorithm")
	})
