...
import (
	...
	"github.com/trustbloc/did-go/method/sidetreelongform/sidetree-core/versions/1_0/doctransformer/metadata"
)
...
func TestTransformDocument(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		serviceEndpoint := serviceEndpointEntry.(map[string]interface{}) //nolint: errcheck
		...
		expectedPublicKeys := []string{
			"master", "general", "authentication", "assertion", "agreement", "delegation", "invocation"}
	...
	t.Run("success - include operations (published/unpublished)", func(t *testing.T) {
		...
		require.Equal(t, 2, len(
			methodMetadata[document.PublishedOperationsProperty].([]*metadata.PublishedOperation)))
		require.Equal(t, 1, len(
			methodMetadata[document.UnpublishedOperationsProperty].([]*metadata.UnpublishedOperation)))
	})
...
func TestWithBase(t *testing.T) {
	...
	baseMap := didDoc.Context()[1].(map[string]interface{}) //nolint: errcheck
	baseMap["@base"] = testID
