...
func (s *Applier) applyCreateOperation(anchoredOp *operation.AnchoredOperation,
	...
	if err != nil {
		logger.Info("Delta doesn't match delta hash; set update commitment to nil and advance recovery commitment",
			log.WithError(err),
			logfields.WithSuffix(anchoredOp.UniqueSuffix),
			logfields.WithOperationType(string(anchoredOp.Type)),
			logfields.WithTransactionTime(anchoredOp.TransactionTime),
			logfields.WithTransactionNumber(anchoredOp.TransactionNumber))
	...
	if err != nil {
		logger.Info("Apply patches failed; advance commitments",
			log.WithError(err), logfields.WithSuffix(anchoredOp.UniqueSuffix),
			logfields.WithOperationType(string(anchoredOp.Type)),
			logfields.WithTransactionTime(anchoredOp.TransactionTime),
			logfields.WithTransactionNumber(anchoredOp.TransactionNumber))
...
func (s *Applier) applyUpdateOperation(anchoredOp *operation.AnchoredOperation,
	...
	err = s.verifyAnchoringTimeRange(
		signedDataModel.AnchorFrom, signedDataModel.AnchorUntil, anchoredOp.TransactionTime)
	if err != nil {
...
func (s *Applier) applyDeactivateOperation(anchoredOp *operation.AnchoredOperation,
	rm *protocol.ResolutionModel) (*protocol.ResolutionModel, error) { //nolint:funlen
...
func (s *Applier) applyRecoverOperation(anchoredOp *operation.AnchoredOperation,
	...
	if err != nil {
		logger.Info(
			"Recover delta doesn't match delta hash; set update commitment to nil and advance recovery commitment",
			logfields.WithSuffixes(op.UniqueSuffix), logfields.WithOperationType(string(op.Type)),
			logfields.WithTransactionTime(anchoredOp.TransactionTime),
			logfields.WithTransactionNumber(anchoredOp.TransactionNumber),
			log.WithError(err))
