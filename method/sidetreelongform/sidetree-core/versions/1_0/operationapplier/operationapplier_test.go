...
func TestApplier_Apply(t *testing.T) {
	...
	t.Run("apply recover to non existing document error", func(t *testing.T) {
		...
		recoverOp, _, err := getAnchoredRecoverOperation(
			recoveryKey, updateKey, createOp.UniqueSuffix, 2)
		require.NoError(t, err)
...
func TestUpdateDocument(t *testing.T) {
	...
	t.Run("error -  operation with reused next commitment", func(t *testing.T) {
		...
		require.EqualError(t, err,
			"update delta doesn't match delta hash: supplied hash doesn't match original content")
	})
	...
	t.Run("unmarshal signed data model error", func(t *testing.T) {
		...
		require.Contains(t, err.Error(),
			"failed to parse update operation in batch mode: failed to unmarshal signed data model for update")
	})
...
func TestDeactivate(t *testing.T) {
	...
	t.Run("unmarshal signed data model error", func(t *testing.T) {
		...
		require.Nil(t, rm)
		require.Contains(t, err.Error(),
			"failed to parse deactive operation in batch mode: failed to unmarshal signed data model for deactivate")
	})
	...
	t.Run("did suffix doesn't match signed value error", func(t *testing.T) {
		...
		require.Nil(t, rm)
		require.Contains(t, err.Error(),
			"failed to parse deactive operation in batch mode: signed did suffix mismatch for deactivate")
	})
	...
	t.Run("invalid anchoring time range - anchor until time is less then anchoring time", func(t *testing.T) {
		...
		require.Nil(t, rm)
		require.Contains(t, err.Error(),
			"invalid anchoring time range: anchor until time is less then anchoring time")
	})
}
...
func TestRecover(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		recoverOp, nextRecoveryKey, err := getAnchoredRecoverOperation(
			recoveryKey, updateKey, uniqueSuffix, 1)
		require.NoError(t, err)
	...
	t.Run("unmarshal signed data model error", func(t *testing.T) {
		...
		require.Nil(t, rm)
		require.Contains(t, err.Error(),
			"failed to parse recover operation in batch mode: failed to unmarshal signed data model for recover")
	})
...
func getUpdateOperation(privateKey *ecdsa.PrivateKey, uniqueSuffix string, operationNumber uint,
...
func getAnchoredUpdateOperation(
	privateKey *ecdsa.PrivateKey, uniqueSuffix string, operationNumber uint,
...
func getUpdateOperationWithSigner(
	s client.Signer, privateKey *ecdsa.PrivateKey, uniqueSuffix string,
	operationNumber uint) (*model.Operation, *ecdsa.PrivateKey, error) {
	p := map[string]interface{}{
		"op":    "replace",
		"path":  "/test",
...
func getDeactivateOperation(privateKey *ecdsa.PrivateKey, uniqueSuffix string,
...
func getAnchoredRecoverOperation(recoveryKey, updateKey *ecdsa.PrivateKey, uniqueSuffix string, operationNumber uint,
) (*operation.AnchoredOperation, *ecdsa.PrivateKey, error) {
...
func getRecoverOperationWithSigner(signer client.Signer, recoveryKey, updateKey *ecdsa.PrivateKey, uniqueSuffix string,
) (*model.Operation, *ecdsa.PrivateKey, error) {
...
func getRecoverRequest(signer client.Signer, delta *model.DeltaModel, signedDataModel *model.RecoverSignedDataModel,
...
func getDefaultRecoverRequest(signer client.Signer, recoveryKey, updateKey *ecdsa.PrivateKey,
