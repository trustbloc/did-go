...
func TestParseRecoverOperation(t *testing.T) {
	...
	t.Run("parse recoverRequest request error", func(t *testing.T) {
		schema, err := parser.ParseRecoverOperation([]byte(""), false)
		require.Error(t, err)
		require.Nil(t, schema)
		require.Contains(t, err.Error(), "unexpected end of JSON input")
	})
	t.Run("validate recoverRequest request", func(t *testing.T) {
	...
	t.Run("validate signed data error", func(t *testing.T) {
		...
		require.Contains(t, err.Error(),
			"validate signed data for recovery: signing key validation failed: JWK crv is missing")
		require.Nil(t, op)
	})
	...
	t.Run("error - update commitment equals recovery commitment", func(t *testing.T) {
		...
		require.Error(t, err)
		require.Contains(t, err.Error(),
			"recovery and update commitments cannot be equal, re-using public keys is not allowed")
		require.Nil(t, op)
	})
...
func TestValidateSignedDataForRecovery(t *testing.T) {
	...
	t.Run("invalid patch data hash", func(t *testing.T) {
		...
		require.Error(t, err)
		require.Contains(t, err.Error(),
			"delta hash is not computed with the required hash algorithms: [18]")
	})
	t.Run("invalid next recovery commitment hash", func(t *testing.T) {
		...
		require.Error(t, err)
		require.Contains(t, err.Error(),
			"recovery commitment is not computed with the required hash algorithms: [18]")
	})
...
func TestParseSignedData(t *testing.T) {
	...
	t.Run("error - invalid signing algorithm", func(t *testing.T) {
		...
		require.Nil(t, jws)
		require.Contains(t, err.Error(),
			"failed to parse signed data: algorithm 'alg' is not in the allowed list [other]")
	})
}
...
func TestValidateSigningKey(t *testing.T) {
	...
	t.Run("error - failed to decode signing key nonce", func(t *testing.T) {
		...
		require.Error(t, err)
		require.Contains(t, err.Error(),
			"validate signing key nonce: failed to decode nonce 'nonce': illegal base64 data")
	})
	...
	t.Run("error - failed to validate nonce size", func(t *testing.T) {
		...
		require.Error(t, err)
		require.Contains(t, err.Error(),
			"validate signing key nonce: nonce size '5' doesn't match configured nonce size '16'")
	})
...
func TestValidateRecoverRequest(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		err = parser.validateRecoverRequest(recoverRequest)
		require.NoError(t, err)
	})
	t.Run("missing signed data", func(t *testing.T) {
		recoverRequest, err := getDefaultRecoverRequest()
		require.NoError(t, err)
		recoverRequest.SignedData = ""
	...
	t.Run("invalid reveal value", func(t *testing.T) {
		recoverRequest, err := getDefaultRecoverRequest()
		require.NoError(t, err)
		recoverRequest.RevealValue = "reveal"
...
func getRecoverRequest(delta *model.DeltaModel, signedData *model.RecoverSignedDataModel,
