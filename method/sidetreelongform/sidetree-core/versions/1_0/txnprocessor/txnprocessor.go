...
func (p *TxnProcessor) Process(sidetreeTxn txn.SidetreeTxn, suffixes ...string) (int, error) {
	logger.Debug("Processing sidetree txn for suffixes", logfields.WithSidetreeTxn(sidetreeTxn),
		logfields.WithSuffixes(suffixes...))
...
func (p *TxnProcessor) processTxnOperations(
	txnOps []*operation.AnchoredOperation, sidetreeTxn *txn.SidetreeTxn) (int, error) {
	...
	for _, op := range txnOps {
		...
		if ok {
			logger.Warn("Duplicate suffix found in transaction operations: discarding operation",
				logfields.WithNamespace(sidetreeTxn.Namespace),
				logfields.WithSuffix(op.UniqueSuffix), logfields.WithOperation(op))
	...
	if err != nil {
		return 0, fmt.Errorf(
			"failed to delete unpublished operations for anchor string[%s]: %w", sidetreeTxn.AnchorString, err)
	}

	return len(ops), nil
}

func updateAnchoredOperation(op *operation.AnchoredOperation, sidetreeTxn *txn.SidetreeTxn,
