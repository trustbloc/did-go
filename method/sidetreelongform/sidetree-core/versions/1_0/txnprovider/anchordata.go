...
func ParseAnchorData(data string) (*AnchorData, error) {
	...
	if len(parts) != allowedParts {
		return nil, fmt.Errorf(
			"parse anchor data[%s] failed: expecting [%d] parts, got [%d] parts",
			data, allowedParts, len(parts))
	}

	ok := integerRegex.MatchString(parts[0])
	if !ok {
		return nil, fmt.Errorf(
			"parse anchor data[%s] failed: number of operations must be positive integer", data)
	}
