...
// nolint:funlen
func (h *OperationHandler) PrepareTxnFiles(ops []*operation.QueuedOperation,
) (*protocol.AnchoringInfo, error) {
	...
	if len(parsedOps.Deactivate) != len(ops) {
		...
		provisionalIndexURI, innerErr = h.createProvisionalIndexFile(
			[]string{chunkURI}, provisionalProofURI, parsedOps.Update)
		if innerErr != nil {
			return nil, innerErr
		}
...
func (h *OperationHandler) parseOperations(ops []*operation.QueuedOperation,
) (*models.SortedOperations, *additionalAnchoringInfo, error) {
	if len(ops) == 0 {
	...
	for _, queuedOperation := range ops {
		...
		if ok {
			logger.Debug("Additional operation found in batch operations - adding operation"+
				" to additional queue to be processed in the next batch",
...
// createCoreIndexFile will create core index file from operations, proof files and provisional index file and write
// it to CAS. Returns core index file address.
func (h *OperationHandler) createCoreIndexFile(
	coreProofURI, mapURI string, ops *models.SortedOperations) (string, error) {
...
func (h *OperationHandler) createProvisionalIndexFile(
	chunks []string, provisionalURI string, ops []*model.Operation) (string, error) {
