...
func (h *OperationProvider) GetTxnOperations(t *txn.SidetreeTxn) ([]*operation.AnchoredOperation, error) {
	...
	if len(txnOps) != anchorData.NumberOfOperations {
		return nil, fmt.Errorf("number of txn ops[%d] doesn't match anchor string num of ops[%d]",
			len(txnOps), anchorData.NumberOfOperations)
	}
...
func (h *OperationProvider) getBatchFiles(
	cif *models.CoreIndexFile, alternateSources ...string) (*batchFiles, error) {
	var err error
...
func (h *OperationProvider) getProvisionalFiles(
	provisionalIndexURI string, alternateSources ...string) (*provisionalFiles, error) {
	var err error
	...
	if files.ProvisionalIndex.ProvisionalProofFileURI != "" {
		files.ProvisionalProof, err = h.getProvisionalProofFile(
			files.ProvisionalIndex.ProvisionalProofFileURI, alternateSources...)
		if err != nil {
			return nil, err
		}
	}
...
func validateBatchFileCounts(batchFiles *batchFiles) error {
	...
	if batchFiles.CoreIndex.CoreProofFileURI != "" {
		...
		if coreRecoverNum != coreProofRecoverNum {
			return fmt.Errorf(
				"number of recover ops[%d] in core index doesn't match number of recover ops[%d] in core proof",
				coreRecoverNum, coreProofRecoverNum)
		}

		if coreDeactivateNum != coreProofDeactivateNum {
			return fmt.Errorf(
				"number of deactivate ops[%d] in core index doesn't match number of deactivate ops[%d] in core proof",
	...
	if batchFiles.CoreIndex.ProvisionalIndexFileURI != "" { //nolint:nestif
		...
		if batchFiles.ProvisionalIndex.ProvisionalProofFileURI != "" {
			...
			if provisionalUpdateNum != provisionalProofUpdateNum {
				return fmt.Errorf("number of update ops[%d] in provisional index doesn't"+
					" match number of update ops[%d] in provisional proof",
		...
		if expectedDeltaCount != len(batchFiles.Chunk.Deltas) {
			return fmt.Errorf(
				"number of create+recover+update operations[%d] doesn't match number of deltas[%d]",
...
func (h *OperationProvider) assembleAnchoredOperations(
	batchFiles *batchFiles, t *txn.SidetreeTxn) ([]*operation.AnchoredOperation, error) {
	...
	logger.Debug("Successfully parsed core index operations",
		logfields.WithTotalCreateOperations(len(cifOps.Create)),
		logfields.WithTotalRecoverOperations(len(cifOps.Recover)),
		logfields.WithTotalDeactivateOperations(len(cifOps.Deactivate)))
	...
	logger.Debug("Successfully parsed provisional index operations",
		logfields.WithTotalUpdateOperations(len(pifOps.Update)))
	...
	if len(operations) != len(batchFiles.Chunk.Deltas) {
		...
		return nil,
			fmt.Errorf("number of create+recover+update operations[%d] doesn't match number of deltas[%d]",
...
func (h *OperationProvider) getCoreIndexFile(
	uri string, alternateSources ...string) (*models.CoreIndexFile, error) { //nolint:dupl
...
func (h *OperationProvider) getCoreProofFile(uri string, alternateSources ...string) (*models.CoreProofFile, error) { //nolint:dupl,lll
...
func (h *OperationProvider) getChunkFile(uri string, alternateSources ...string,
) (*models.ChunkFile, error) { //nolint:dupl
...
//nolint:funlen
func (h *OperationProvider) parseCoreIndexOperations(cif *models.CoreIndexFile, t *txn.SidetreeTxn,
) (*coreOperations, error) {
	if cif.Operations == nil {
		// nothing to do
...
func (h *OperationProvider) readFromAlternateCASSources(casURI string, sources []string) ([]byte, error) {
	for _, source := range sources {
		...
		if err == nil {
			logger.Debug("Successfully retrieved CAS content from alternate source",
				logfields.WithSource(casURIForSource))

			return b, nil
		}

		logger.Warn("Error retrieving CAS content from alternate source",
			logfields.WithSource(casURIForSource), log.WithError(err))
	}
