...
func TestVDRI_Read(t *testing.T) {
	t.Run("test success", func(t *testing.T) {
		...
		require.Equal(t,
			docResolution.DocumentMetadata.EquivalentID[0],
			fmt.Sprintf("did:%s:%s", defaultDIDMethod, didSuffix))
	})
...
func (dh *mockDocHandler) ResolveDocument(_ string, _ ...document.ResolutionOption) (
